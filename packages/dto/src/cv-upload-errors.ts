 ...
 * Provides context about constraints and the actual values that caused the error.
 */
export interface CvUploadErrorDetails {
  ...
  currentSize?: number;
  /** The actual MIME type of the uploaded file */
  currentType?: string;
}

/**
 * Standard error response structure for CV upload failures.
 * Used for consistent error communication between frontend and backend.
 */
...
/**
 * Type for CV upload constraints to enable type-safe access.
 */
 ...
 */
export function isCvUploadError(error: unknown): error is CvUploadErrorResponse {
  return (
    typeof error === 'object' &&
    error !== null &&
    'code' in error &&
